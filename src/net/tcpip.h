...
public:
  ...
  //{ {{
  {
    if (who->protocol_type() == IP &&
        !memcmp(&addr.sin_addr, &((ip_address const *)who)->addr.sin_addr,
  ...
  //{ {{
  {
    char buf[100];
    unsigned char *c = (unsigned char *)(&addr.sin_addr.s_addr);
    sprintf(buf, "%d.%d.%d.%d:%d", c[0], c[1], c[2], c[3],
            htons(addr.sin_port));
...
public:
  ...
  net_address *get_node_address(char const *&server_name, int def_port,
                                int force_port);
  net_socket *connect_to_server(
      net_address *addr,
  ...
  virtual net_address *
  find_address(int port, char *name); // name should be a 256 byte buffer
